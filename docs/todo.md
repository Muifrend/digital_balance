...
# Task: Rewrite README Installation Guide

- [x] Inspect repository files and runtime flow to capture true setup requirements
- [x] Rewrite `README.md` with complete install and first-run instructions
- [x] Verify README commands/paths against code and scripts
- [x] Add review notes for this task

## Review
- Replaced the template README with a setup guide tailored to FocusLens architecture and startup flow.
- Added explicit install steps for:
  - Node dependencies (`npm install`)
  - Python virtualenv creation at project root (`.venv`)
  - sidecar dependency install (`openai`)
  - root `.env` creation with `OPENAI_API_KEY`
- Documented ActivityWatch binary requirements exactly as consumed by `src/main/index.ts`:
  - platform directory names: `macos`, `linux`, `windows`
  - required binaries: `aw-server` and `aw-watcher-window` under each platform folder
- Included run, build, and troubleshooting sections with ports and common startup errors.
